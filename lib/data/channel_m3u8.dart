import 'package:ww_open_tv/models/channel_model.dart';

List<ChannelModel> channelList = [
  //Afghanistan af
  ChannelModel(
    countryCode: 'af',
    name: 'AMC',
    url: 'https://amchls.wns.live/hls/stream.m3u8',
    imageUrl: 'https://i.imgur.com/qRrOi3t.png',
  ),
  ChannelModel(
    countryCode: 'af',
    name: 'ATN International',
    url: 'https://d10rltuy0iweup.cloudfront.net/ATNINT/myStream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/HIPeUDL.png',
  ),
  ChannelModel(
    countryCode: 'af',
    name: 'ATN National',
    url: 'https://d10rltuy0iweup.cloudfront.net/ATNNAT/myStream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/HIPeUDL.png',
  ),
  ChannelModel(
    countryCode: 'af',
    name: 'ATN News',
    url: 'https://d10rltuy0iweup.cloudfront.net/ATNNEWS/myStream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/gaGpLPV.png',
  ),
  ChannelModel(
    countryCode: 'af',
    name: 'Dunya Naw TV',
    url: 'https://dunyanhls.wns.live/hls/stream.m3u8',
    imageUrl: 'https://i.ibb.co/nQkLgyn/dunya-naw-tv.png',
  ),

  //Albania
  ChannelModel(
    countryCode: 'al',
    name: 'AlbKanale Music TV',
    url: 'https://albportal.net/albkanalemusic.m3u8',
    imageUrl: 'https://i.imgur.com/JdKxscs.png',
  ),
  ChannelModel(
    countryCode: 'al',
    name: 'Kanali 7',
    url: 'https://fe.tring.al/delta/105/out/u/1200_1.m3u8',
    imageUrl: 'https://i.imgur.com/rL2v9pM.png',
  ),
  ChannelModel(
    countryCode: 'al',
    name: 'News 24',
    url: 'https://tv.balkanweb.com/news24/livestream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zDwIsO3.png',
  ),

  //Algeria
  ChannelModel(
    countryCode: 'dz',
    name: 'Canal Algérie',
    url: 'https://bozztv.com/inim03/live1/giniko_canalalgerie/chunks.m3u8',
    imageUrl: 'https://i.imgur.com/VEb631f.png',
  ),

  //Angola
  ChannelModel(
    countryCode: 'ao',
    name: 'KK TV',
    url: 'https://w1.manasat.com/ktv-angola/smil:ktv-angola.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/gTabgkN.jpg',
  ),
  ChannelModel(
    countryCode: 'ao',
    name: 'Muzangala TV',
    url:
        'https://5cf4a2c2512a2.streamlock.net/tvmuzangala/tvmuzangala/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/fBeaJoS.png',
  ),

  //Argentina
  ChannelModel(
    countryCode: 'ar',
    name: 'Aire de Santa Fe',
    url:
        'https://unlimited1-us.dps.live/airedesantafetv/airedesantafetv.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/60vSWW0.png',
  ),
  ChannelModel(
    countryCode: 'ar',
    name: 'AlternaTV',
    url: 'https://alternatv.ar/stream/hls/live.m3u8',
    imageUrl: 'https://alternatv.ar/alternatv.png',
  ),

  ChannelModel(
    countryCode: 'ar',
    name: 'Antena 91',
    url: 'https://tuvideoonline.com.ar:3393/live/antena91live.m3u8',
    imageUrl: 'https://i.imgur.com/HAJxHcV.png',
  ),
  ChannelModel(
    countryCode: 'ar',
    name: 'Argentinísima Satelital',
    url: 'https://stream1.sersat.com/hls/argentinisima.m3u8',
    imageUrl: 'https://i.imgur.com/xFgJawa.png',
  ),
  ChannelModel(
    countryCode: 'ar',
    name: 'Misiones Cuatro',
    url: 'https://streamlov.alsolnet.com/misionescuatro/live/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/HXRpRlK.png',
  ),

  //Armenia
  ChannelModel(
    countryCode: 'am',
    name: 'ARTN TV',
    url: 'https://streamer1.connectto.com/ARTN_mobile/index.m3u8',
    imageUrl: 'https://i.imgur.com/xd6zuuw.png',
  ),

  ChannelModel(
    countryCode: 'am',
    name: 'First Channel News',
    url: 'https://amtvusdvr.tulix.tv/am3abr/index.m3u8',
    imageUrl: 'https://i.imgur.com/RlYJUH4.png',
  ),

//Aruba
  ChannelModel(
    countryCode: 'aw',
    name: 'Cool FM 98.9',
    url: 'https://live2.tensila.com/cool-v-1.arubara/hls/master.m3u8',
    imageUrl: 'https://i.imgur.com/uGE6lZ2.png',
  ),
  ChannelModel(
    countryCode: 'aw',
    name: 'Heart Radio Aruba',
    url: 'https://live2.tensila.com/heart-v-1.heartar/hls/live/mystream.m3u8',
    imageUrl: 'https://i.imgur.com/MKZCskA.png',
  ),
  ChannelModel(
    countryCode: 'aw',
    name: 'VIP TV',
    url: 'https://ed5ov1.live.opencaster.com/bkyqeDgfaukC/index.m3u8',
    imageUrl: 'https://i.imgur.com/6N0qITv.png',
  ),

  //Australia
  ChannelModel(
    countryCode: 'au',
    name: 'ABC Me',
    url: 'https://c.mjh.nz/abc-me.m3u8',
    imageUrl: 'https://i.imgur.com/6NyFLAm.png',
  ),
  ChannelModel(
    countryCode: 'au',
    name: 'Bloomberg TV',
    url: 'https://bloomberg.com/media-manifest/streams/aus.m3u8',
    imageUrl: 'https://i.imgur.com/OuogLHx.png',
  ),
  ChannelModel(
    countryCode: 'au',
    name: 'Channel 44',
    url: 'https://d1k6kax80wecy5.cloudfront.net/WFqZJc/index.m3u8',
    imageUrl: 'https://i.imgur.com/P2wUGh9.png',
  ),
  ChannelModel(
    countryCode: 'au',
    name: 'Hope Channel',
    url:
        'https://videodelivery.net/9fb3596948ddf463fde0ec4b85625b24/manifest/video.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/en/9/9d/HopeChannelFVweb.png',
  ),
  ChannelModel(
    countryCode: 'au',
    name: 'Racing',
    url:
        'https://racingvic-i.akamaized.net/hls/live/598695/racingvic/index1500.m3u8',
    imageUrl: 'https://i.imgur.com/Q55HX1O.png',
  ),
  ChannelModel(
    countryCode: 'au',
    name: 'Ticker News',
    url:
        'https://cdn-uw2-prod.tsv2.amagi.tv/linear/amg01486-tickernews-tickernewsweb-ono/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/z7M0QxV.png',
  ),
  ChannelModel(
    countryCode: 'au',
    name: 'TVSN',
    url:
        'https://tvsnhlslivetest.akamaized.net/hls/live/2034711/TVSN-MSL4/master.m3u8',
    imageUrl: 'https://i.imgur.com/lmoAFm3.png',
  ),

  //Austria
  ChannelModel(
    countryCode: 'at',
    name: 'FUEL TV',
    url:
        'https://d35j504z0x2vu2.cloudfront.net/v1/manifest/0bc8e8376bd8417a1b6761138aa41c26c7309312/fuel-tv/606c2f67-acff-4152-975a-e5bfef54eb61/2.m3u8',
    imageUrl: 'https://i.imgur.com/I8mviBy.png',
  ),
  ChannelModel(
    countryCode: 'at',
    name: 'K-TV',
    url: 'https://d1pz8zear993v8.cloudfront.net/hlsme/kathtv.m3u8',
    imageUrl: 'https://i.imgur.com/RYbR4Sf.png',
  ),
  ChannelModel(
    countryCode: 'at',
    name: 'Kronehit',
    url: 'https://bitcdn-kronehit.bitmovin.com/v2/hls/playlist.m3u8',
    imageUrl: '',
  ),
  ChannelModel(
    countryCode: 'at',
    name: 'Okto TV',
    url:
        'https://cdn3.wowza.com/1/MHFtazJReW5rOFhP/N2NWNDZ2/hls/live/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/9XfhgbS.png',
  ),
  ChannelModel(
    countryCode: 'at',
    name: 'Red Bull TV',
    url: 'https://rbmn-live.akamaized.net/hls/live/590964/BoRB-AT/master.m3u8',
    imageUrl: 'https://i.postimg.cc/8c78JM28/image.png',
  ),
  ChannelModel(
    countryCode: 'at',
    name: 'TV 1 Oberösterreich',
    url:
        'https://streaming10.huberwebmedia.at/LiveApp/streams/238985177034727564068269_adaptive.m3u8',
    imageUrl: 'https://i.imgur.com/KbMkEIn.png',
  ),

  //Azerbaijan
  ChannelModel(
    countryCode: 'az',
    name: 'AZ News',
    url: 'https://edge1.socialsmart.tv/aznews/smil/aznews/720p/chunks.m3u8',
    imageUrl: 'https://i.imgur.com/Kw17XfP.png',
  ),
  ChannelModel(
    countryCode: 'az',
    name: 'Az TV',
    url: 'https://str.yodacdn.net/aztv/index.m3u8',
    imageUrl: 'https://i.imgur.com/snBMMeH.png',
  ),

  //Bangladesh
  ChannelModel(
    countryCode: 'bd',
    name: 'Ekushey TV',
    url: 'https://ekusheyserver.com/etvlivesn.m3u8',
    imageUrl: 'https://i.imgur.com/lRpkGHj.png',
  ),

//Barbados
  ChannelModel(
    countryCode: 'bb',
    name: 'CBC TV8',
    url: 'https://1740288887.rsc.cdn77.org/1740288887/index.m3u8',
    imageUrl: 'https://i.imgur.com/WsD83lq.png',
  ),

  //Belarus
  ChannelModel(
    countryCode: 'by',
    name: 'СТБ',
    url: 'https://ctv.dc.beltelecom.by/ctv/ctv.stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/DCbfsyW.png',
  ),

  //Belgium
  ChannelModel(
    countryCode: 'be',
    name: 'Bel RTL',
    url:
        'https://bel-live-hls.akamaized.net/hls/live/2038650/BEL-Live-HLS/master.m3u8',
    imageUrl: 'https://i.imgur.com/vJlAZsV.png',
  ),
  ChannelModel(
    countryCode: 'be',
    name: 'Afrika Media TV',
    url: 'https://goccn.cloud/hls/belafrikatv/index.m3u8',
    imageUrl: 'https://i.imgur.com/9UmR6cs.png',
  ),
  ChannelModel(
    countryCode: 'be',
    name: 'LN24',
    url: 'https://live-ln24.digiteka.com/1911668011/index.m3u8',
    imageUrl: 'https://i.imgur.com/1EkZRwn.png',
  ),
  ChannelModel(
    countryCode: 'be',
    name: 'Q-Music',
    url: 'https://dpp-streamlive-plain.medialaancdn.be/qmusic/plain/hls.m3u8',
    imageUrl: 'https://i.imgur.com/VX5e9Xt.png',
  ),
  ChannelModel(
    countryCode: 'be',
    name: 'Stêrk TV',
    url: 'https://muzkurd.com/sterktv/sterk/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/bIheNAI.png',
  ),

  ChannelModel(
    countryCode: 'be',
    name: 'VTM',
    url:
        'https://dpp-live-events.medialaancdn.be/events/hls/aes/webstream1.m3u8',
    imageUrl: 'https://i.imgur.com/djkPc9w.png',
  ),
  ChannelModel(
    countryCode: 'be',
    name: 'VTM 2',
    url:
        'https://dpp-live-events.medialaancdn.be/events/hls/aes/webstream3.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/4/42/VTM_2_Logo.png/512px-VTM_2_Logo.png',
  ),

  //Benin
  ChannelModel(
    countryCode: 'bj',
    name: 'Bénin Web TV',
    url: 'https://vdo.pro-fhi.net:3244/stream/play.m3u8',
    imageUrl: 'https://i.imgur.com/CYy4n0N.png',
  ),
  ChannelModel(
    countryCode: 'bj',
    name: 'Canal 3 Bénin',
    url: 'https://live.creacast.com/bluediamond/stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/WIrVAa2.png',
  ),

  //Bolivia
  ChannelModel(
    countryCode: 'bo',
    name: 'Abya Yala TV',
    url: 'https://master.tucableip.com/abyayalatv/playlist.m3u8',
    imageUrl: 'https://comteco.com.bo/img/upload/canales/abya-yala.png',
  ),
  ChannelModel(
    countryCode: 'bo',
    name: 'MegaTV',
    url: 'https://solo.disfrutaenlared.com:1936/tvcbba/tvcbba/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/3d4uiZF.png',
  ),
  ChannelModel(
    countryCode: 'bo',
    name: 'TV Show Oruro',
    url: 'http://tv.istbolivia.com:1935/tvshow/tvshow/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/nnsYJCq/IMG-20210731-094609.png',
  ),

  //Bonaire
  ChannelModel(
    countryCode: 'bq',
    name: 'Live99FM',
    url: 'https://media.streambrothers.com:19360/8022/8022.m3u8',
    imageUrl: 'https://i.imgur.com/U43UrQT.png',
  ),

  //Bosnia and Herzegovina
  ChannelModel(
    countryCode: 'ba',
    name: 'BN TV',
    url: 'https://rtvbn.tv:8080/live/index.m3u8',
    imageUrl: 'https://i.imgur.com/DUBvfWb.png',
  ),
  ChannelModel(
    countryCode: 'ba',
    name: 'RTV Zenica',
    url: 'https://stream.rtvze.ba/live/123/123.m3u8',
    imageUrl: 'https://i.imgur.com/TKUaflB.png',
  ),

  //Brazil

  ChannelModel(
    countryCode: 'br',
    name: 'Adesso TV',
    url:
        'https://cdn.jmvstream.com/w/LVW-9715/LVW9715_12B26T62tm/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/KgetM8j.pn',
  ),
  ChannelModel(
    countryCode: 'br',
    name: 'All Sports',
    url: 'https://5cf4a2c2512a2.streamlock.net/dgrau/dgrau/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/wULpnYR.png',
  ),
  ChannelModel(
    countryCode: 'br',
    name: 'Bem TV',
    url: 'http://wz4.dnip.com.br/bemtv/bemtv.sdp/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/ifGQGJx.png',
  ),
  ChannelModel(
    countryCode: 'br',
    name: 'Canal Educação',
    url: 'https://canaleducacao-stream.ebc.com.br/index.m3u8',
    imageUrl: 'https://i.imgur.com/OOB7nrS.png',
  ),
  ChannelModel(
    countryCode: 'br',
    name: 'Canal Gov',
    url: 'https://canalgov-stream.ebc.com.br/index.m3u8',
    imageUrl: 'https://i.imgur.com/rHPY0Yv.png',
  ),
  ChannelModel(
    countryCode: 'br',
    name: 'Novo Tempo',
    url:
        'https://stream.live.novotempo.com/tv/smil:tvnovotempo.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/exdDHml.png',
  ),

  //British Virgin Islands
  ChannelModel(
    countryCode: 'vg',
    name: 'CBN TV',
    url: 'https://5790d294af2dc.streamlock.net/CBNVI/CBNVI/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/uHvE04N.png',
  ),

  //Bulgaria
  ChannelModel(
    countryCode: 'bg',
    name: 'Evrokom',
    url: 'https://live.ecomservice.bg/hls/stream.m3u8',
    imageUrl: 'https://i.imgur.com/8LPtPf6.png',
  ),
  ChannelModel(
    countryCode: 'bg',
    name: 'Hope Channel',
    url: 'https://hc1.hopetv.bg/live/hopetv_all.smil/playlist.m3u8',
    imageUrl:
        'https://www.lyngsat.com/logo/tv/hh/hope-channel-bulgaria-us-bg.png',
  ),
  ChannelModel(
    countryCode: 'bg',
    name: 'RM TV',
    url: 'https://transcoder1.bitcare.eu/streaming/rimextv/rmtv.m3u8',
    imageUrl: 'https://rmtv.bg/img/logo.png',
  ),

  //Burkina Faso
  ChannelModel(
    countryCode: 'bf',
    name: 'Burkina Info TV',
    url: 'http://69.64.57.208/burkinainfo/index.m3u8',
    imageUrl: 'https://i.imgur.com/a9v5CbU.png',
  ),

  //Cambodia
  ChannelModel(
    countryCode: 'kh',
    name: 'Apsara TV11',
    url: 'https://live.ams.com.kh/app/stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/a9v5CbU.png',
  ),
  ChannelModel(
    countryCode: 'kh',
    name: 'Bayon TV',
    url: 'https://live.kh.malimarcdn.com/live/bayonhd.stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Gry2KEN.jpg',
  ),
  ChannelModel(
    countryCode: 'kh',
    name: 'Netlink TV',
    url: 'https://netlink.netlinkbroadcaster.com/hls/test.m3u8',
    imageUrl: 'https://i.imgur.com/SoBhuIb.png',
  ),

  //Cameroon
  ChannelModel(
    countryCode: 'cm',
    name: 'CEN Télévision',
    url: 'https://strhlslb01.streamakaci.tv/cen/cen-multi/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/2vJfdgN/unnamed.jpg',
  ),
  ChannelModel(
    countryCode: 'cm',
    name: 'CTV Afrique',
    url: 'https://stream.it-innov.com/ctv/index.m3u8',
    imageUrl: 'https://i.ibb.co/k0mws9B/Ctv-Afrique.jpg',
  ),

  //Canada

  ChannelModel(
    countryCode: 'ca',
    name: 'Afghan Nobel Movies',
    url:
        'https://live.relentlessinnovations.net:1936/afghannobel/afghannobel/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/zfdbVSm/logo-1.png',
  ),
  ChannelModel(
    countryCode: 'ca',
    name: 'Afghan Nobel TV',
    url:
        'https://live.relentlessinnovations.net:1936/afghannobeltv/afghannobeltv/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/0Mb1ZQC/IMG-20230628-133738.jpg',
  ),
  ChannelModel(
    countryCode: 'ca',
    name: 'Amazing Discoveries TV',
    url: 'https://uni01rtmp.tulix.tv/amazingdtv/amazingdtv/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/I0oPFTj.png',
  ),
  ChannelModel(
    countryCode: 'ca',
    name: 'Assemblée Nationale du Québec',
    url:
        'https://cdn3.wowza.com/5/SVEySlNEQ0FOWXlS/diffusion/canal17/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Eta6gUq.png',
  ),
  ChannelModel(
    countryCode: 'ca',
    name: 'Az Star TV',
    url: 'https://live.livestreamtv.ca/azstar/ngrp:azstar/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/di3XX5L.png',
  ),
  ChannelModel(
    countryCode: 'ca',
    name: 'Canal Savoir',
    url: 'https://hls.savoir.media/live/stream.m3u8',
    imageUrl: 'https://i.imgur.com/QBzvCJc.png',
  ),

  //Cape Verde
  ChannelModel(
    countryCode: 'cv',
    name: 'TV Cidade',
    url: 'https://stmv1.srvif.com/tvcidade/tvcidade/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/gmSu21X.jpg',
  ),

  //Chad
  ChannelModel(
    countryCode: 'td',
    name: 'Télé Tchad',
    url:
        'https://strhlslb01.streamakaci.tv/str_tchad_tchad/str_tchad_multi/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/udSzfHD.png',
  ),

  //Chile
  ChannelModel(
    countryCode: 'cl',
    name: '13 Entretención',
    url:
        'https://origin.dpsgo.com/ssai/event/BBp0VeP6QtOOlH8nu3bWTg/master.m3u8',
    imageUrl: 'https://i.imgur.com/utI1tJV.png',
  ),
  ChannelModel(
    countryCode: 'cl',
    name: '13 Festival',
    url:
        'https://origin.dpsgo.com/ssai/event/Nftd0fM2SXasfDlRphvUsg/master.m3u8',
    imageUrl: 'https://i.imgur.com/hTUe0Pw.png',
  ),

  //China
  ChannelModel(
    countryCode: 'cn',
    name: 'Bread TV面包台',
    url: 'https://video.bread-tv.com:8091/hls-live24/online/index.m3u8',
    imageUrl: 'https://i.imgur.com/UR9vEIN.png',
  ),
  ChannelModel(
    countryCode: 'cn',
    name: 'VOA美国之音',
    url:
        'https://voa-ingest.akamaized.net/hls/live/2033878/tvmc08/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/XCTZkoq.png',
  ),

//Colombia
  ChannelModel(
    countryCode: 'co',
    name: 'Agenda Al Día TV',
    url: 'https://cp.panelchs.com:1936/8156/8156/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/JFEFVwV.jpg',
  ),
  ChannelModel(
    countryCode: 'co',
    name: 'ANC Televisión',
    url: 'https://canal.mediaserver.com.co/live/ANCTELEVISION.m3u8',
    imageUrl: 'https://i.imgur.com/xjTU14t.jpg',
  ),

  //Costa Rica

  ChannelModel(
    countryCode: 'cr',
    name: '¡OPA!',
    url: 'https://opa.usastreams.com:3101/live/canalopalive.m3u8',
    imageUrl: 'https://i.imgur.com/300c1ZH.png',
  ),
  ChannelModel(
    countryCode: 'cr',
    name: 'Cristo Visión Canal 31',
    url:
        'https://3a310f6ec721e6b362fcd22772b57f36a2fe6bb2.tabeelcr.com:8080/cristovision31/iptv.m3u8',
    imageUrl: 'https://i.imgur.com/CFXZ8j0.png',
  ),
  ChannelModel(
    countryCode: 'cr',
    name: 'Enlace',
    url: 'https://livecdn.enlace.plus/enlace/smil:enlace-hd.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/YwUVTmS.png',
  ),

  //Croatia
  ChannelModel(
    countryCode: 'hr',
    name: 'CMC TV',
    url: 'https://stream.cmctv.hr:49998/cmc/live.m3u8',
    imageUrl: 'https://i.imgur.com/FCb7Ag2.png',
  ),
  ChannelModel(
    countryCode: 'hr',
    name: 'HRT 1',
    url: 'https://webtvstream.bhtelecom.ba/hrt1.m3u8',
    imageUrl: 'https://i.imgur.com/lDSctzX.png',
  ),
  ChannelModel(
    countryCode: 'hr',
    name: 'HRT 2',
    url: 'https://webtvstream.bhtelecom.ba/hrt2.m3u8',
    imageUrl: 'https://i.imgur.com/yyePcl1.png',
  ),
  ChannelModel(
    countryCode: 'hr',
    name: 'HRT 3',
    url: 'https://webtvstream.bhtelecom.ba/hrt3.m3u8',
    imageUrl: 'https://i.imgur.com/4OrOJRx.png',
  ),
  ChannelModel(
    countryCode: 'hr',
    name: 'Klape i Tambure TV',
    url: 'https://stream.cmctv.hr:49998/kit/live.m3u8',
    imageUrl: 'https://i.imgur.com/4OrOJRx.png',
  ),
  ChannelModel(
    countryCode: 'hr',
    name: 'STV',
    url: 'http://89.201.163.244:8080/hls/hdmi.m3u8',
    imageUrl: 'https://i.imgur.com/VcNrY0m.png',
  ),

  //Cuba
  ChannelModel(
    countryCode: 'cu',
    name: 'Prensa Latina TV',
    url: 'https://eu1.servers10.com:8081/8192/index.m3u8',
    imageUrl: 'https://i.imgur.com/jbm2S9O.jpg',
  ),
  ChannelModel(
    countryCode: 'cu',
    name: 'Radio y Televisión Martí',
    url:
        'https://ocb-ingest.akamaized.net/hls/live/2037032/tvmc01/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Y7E5OTy.png',
  ),

  //Cyprus
  ChannelModel(
    countryCode: 'cy',
    name: 'ADA TV',
    url: 'https://sc-kuzeykibrissmarttv.ercdn.net/adatv/bant1/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/YWmkUWU.png',
  ),

//Czech Republic
  ChannelModel(
    countryCode: 'cz',
    name: 'Elektrika TV',
    url: 'https://rtmp.elektrika.cz/live/myStream.sdp/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/OLd5od5.png',
  ),
  ChannelModel(
    countryCode: 'cz',
    name: 'JČ1',
    url: 'https://stream.jc1.cz/hls/jc1.m3u8',
    imageUrl: 'https://i.imgur.com/AM8YtcV.png',
  ),
  ChannelModel(
    countryCode: 'cz',
    name: 'Minimax',
    url: 'http://88.212.7.10/live/test_minimax/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/WWOlm1x.png',
  ),
  ChannelModel(
    countryCode: 'cz',
    name: 'Óčko',
    url:
        'https://ocko-live-dash.ssl.cdn.cra.cz/cra_live2/ocko.stream.1.smil/playlist.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/2/20/%C3%93%C4%8Dko_logo_2012.png/512px-%C3%93%C4%8Dko_logo_2012.png',
  ),

  //Denmark
  ChannelModel(
    countryCode: 'dk',
    name: 'TV 2 Kosmopol',
    url:
        'https://cdnapi.kaltura.com/p/2045321/sp/204532100/playManifest/entryId/1_2kojfk4m/format/applehttp/protocol/https/uiConfId/32599481/a.m3u8',
    imageUrl: 'https://i.imgur.com/oVmCoKY.png',
  ),
  ChannelModel(
    countryCode: 'dk',
    name: 'TV2 Nord',
    url:
        'https://cdnapi.kaltura.com/p/1956931/sp/195693100/playManifest/entryId/1_h9yfe7h2/protocol/https/format/applehttp/a.m3u8',
    imageUrl: 'https://i.imgur.com/tEJ22UW.png',
  ),
  ChannelModel(
    countryCode: 'dk',
    name: 'TV2 Øst',
    url:
        'https://cdnapi.kaltura.com/p/1953381/sp/195338100/playManifest/entryId/0_zphj9q61/protocol/https/format/applehttp/a.m3u8',
    imageUrl: 'https://i.imgur.com/H9l6Ulw.png',
  ),

  //Dominica
  ChannelModel(
    countryCode: 'dm',
    name: 'TDN TV',
    url: 'https://liveurestreamingcomlive.cdn.ypt.me/62ec67ea435c6.m3u8',
    imageUrl: 'https://i.ibb.co/x5gqtmJ/TDN-TV-tvtolive-com.jpg',
  ),

  //Dominican Republic
  ChannelModel(
    countryCode: 'do',
    name: 'Agenda 56',
    url: 'https://cloudflare.streamgato.us:3148/live/agenda56tvlive.m3u8',
    imageUrl: 'https://i.imgur.com/G7zIVB7.png',
  ),
  ChannelModel(
    countryCode: 'do',
    name: 'Amé 47',
    url: 'https://ss9.domint.net:3040/ame_str/amecanal47/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/MUEbjeP.png',
  ),
  ChannelModel(
    countryCode: 'do',
    name: 'Bonao TV',
    url: 'https://ss5.domint.net:3026/btv_str/bonao/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/en1vCZB.png',
  ),

  //Ecuador
  ChannelModel(
    countryCode: 'ec',
    name: 'América Estéreo Guayaquil',
    url:
        'https://video.makrodigital.com/americaestereoguayaquil/americaestereoguayaquil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/gUqKymM.png',
  ),
  ChannelModel(
    countryCode: 'ec',
    name: 'América Estéreo Ibarra',
    url:
        'https://video.makrodigital.com/americaestereoibarra/americaestereoibarra/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/gUqKymM.png',
  ),

  //Egypt
  ChannelModel(
    countryCode: 'eg',
    name: 'Alfath Atfal TV',
    url:
        'https://alfat7-q.com:5443/LiveApp/streams/434833549141598440488987.m3u8',
    imageUrl: '',
  ),
  ChannelModel(
    countryCode: 'eg',
    name: 'Alfath Sonnah TV',
    url:
        'https://alfat7-q.com:5443/LiveApp/streams/986613792230697141226562.m3u8',
    imageUrl: 'https://i.imgur.com/7VYJStp.png',
  ),

  //El Salvador
  ChannelModel(
    countryCode: 'sv',
    name: 'Canal 3 Impresionante',
    url: 'https://cloud2.streaminglivehd.com:1936/8048/8048/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/mdfiWG9.png',
  ),

  //Equatorial Guinea
  ChannelModel(
    countryCode: 'gq',
    name: 'SolTV',
    url: 'https://stream.solmediaeg.com/soltv/soltv.m3u8',
    imageUrl: 'https://i.imgur.com/TJHk6Va.png',
  ),

  //Ethiopia
  ChannelModel(
    countryCode: 'et',
    name: 'Addis TV',
    url: 'https://rrsatrtmp.tulix.tv/addis1/addis1multi.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/kqHWrZ9.png',
  ),
  ChannelModel(
    countryCode: 'et',
    name: 'EBS Cinema',
    url: 'https://rpn.bozztv.com/ebstv/ebscinema/index.m3u8',
    imageUrl: 'https://i.imgur.com/fewPTFE.png',
  ),
  ChannelModel(
    countryCode: 'et',
    name: 'EBS HD',
    url: 'https://rpn.bozztv.com/ebstv/ebstv/index.m3u8',
    imageUrl: 'https://i.imgur.com/AcZliAE.png',
  ),

//Finland
  ChannelModel(
    countryCode: 'fi',
    name: 'AVA',
    url: 'https://live-fi.tvkaista.net/ava/live.m3u8',
    imageUrl: 'https://i.imgur.com/8TR4noi.png',
  ),
  ChannelModel(
    countryCode: 'fi',
    name: 'Frii',
    url: 'https://live-fi.tvkaista.net/frii/live.m3u8',
    imageUrl: 'https://i.imgur.com/9sYC2IE.png',
  ),
  ChannelModel(
    countryCode: 'fi',
    name: 'Himlen TV7',
    url: 'https://vod.tv7.fi/tv7-se/smil:tv7-se.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/k8a7KNn.png',
  ),

  //France
  ChannelModel(
    countryCode: 'fr',
    name: '20 Minutes TV',
    url:
        'https://lives.digiteka.com/stream/86d3e867-a272-496b-8412-f59aa0104771/index.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/c/cb/20_Minutes_TV_IDF_logo_%282023%29.png/320px-20_Minutes_TV_IDF_logo_%282023%29.png',
  ),
  ChannelModel(
    countryCode: 'fr',
    name: 'A12 TV',
    url: 'https://video1.getstreamhosting.com:1936/8420/8420/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/JgnjgvF.png',
  ),
  ChannelModel(
    countryCode: 'fr',
    name: 'Albi TV',
    url: 'https://live.creacast.com/albi-tv-ch1/stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/MJdY1m3.png',
  ),

  //Georgia
  ChannelModel(
    countryCode: 'ge',
    name: 'Abaza TV',
    url: 'https://public.apsny.camera/tv_abaza_tv_to_site/index.m3u8',
    imageUrl: 'https://i.imgur.com/7Jo7UAf.png',
  ),
  ChannelModel(
    countryCode: 'ge',
    name: 'Formula',
    url: 'https://c4635.cdn.xsg.ge/c4635/TVFormula/index.m3u8',
    imageUrl: 'https://i.imgur.com/fsqBn8G.png',
  ),
  ChannelModel(
    countryCode: 'ge',
    name: 'Imedi TV',
    url: 'https://tv.cdn.xsg.ge/imedihd/index.m3u8',
    imageUrl: 'https://i.imgur.com/94hNyxZ.png',
  ),

  //Germany
  ChannelModel(
    countryCode: 'de',
    name: '1-2-3 TV',
    url: 'https://123tv-mx1.flex-cdn.net/index.m3u8',
    imageUrl: 'https://i.imgur.com/slSUDNX.png',
  ),
  ChannelModel(
    countryCode: 'de',
    name: 'Anixe +',
    url: 'https://ma.anixa.tv/clips/stream/anixesd/index.m3u8',
    imageUrl: 'https://i.imgur.com/hvGJ2oa.png',
  ),
  ChannelModel(
    countryCode: 'de',
    name: 'Anixe HD Serie',
    url: 'https://ma.anixa.tv/clips/stream/anixehd/index.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c0/Anixe_HD.svg/512px-Anixe_HD.svg.png',
  ),
  ChannelModel(
    countryCode: 'de',
    name: 'ARD-alpha',
    url: 'http://livestreams.br.de/i/bralpha_germany@119899/master.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/4/4b/ARD_alpha.svg/512px-ARD_alpha.svg.png',
  ),
  ChannelModel(
    countryCode: 'de',
    name: 'Aristo TV',
    url: 'https://ma.anixa.tv/clips/stream/aristo/index.m3u8',
    imageUrl: 'https://i.imgur.com/DLVjeno.png',
  ),

  //Ghana
  ChannelModel(
    countryCode: 'gh',
    name: 'ChannelOne TV',
    url: 'https://2ctv.crystaltv.mobi:44095/tv877.167.m3u8',
    imageUrl: 'https://i.imgur.com/XJsN2WR.png',
  ),
  ChannelModel(
    countryCode: 'gh',
    name: 'GHtv Holland',
    url: 'http://37.48.120.77:8080/hls/ghtvonline.m3u8',
    imageUrl: 'https://i.imgur.com/hOGS5LL.png',
  ),
  ChannelModel(
    countryCode: 'gh',
    name: 'Hope Channel Ghana',
    url:
        'https://videodelivery.net/dfbdca87f2a6291aa4fdc8fe3290769b/manifest/video.m3u8',
    imageUrl: 'https://i.imgur.com/d1P9ekm.png',
  ),

  //Greece
  ChannelModel(
    countryCode: 'gr',
    name: 'ART TV',
    url: 'https://til.pp.ua:3711/live/arttvgr70live.m3u8',
    imageUrl: 'https://i.imgur.com/ZgKZ5u9.jpg',
  ),
  ChannelModel(
    countryCode: 'gr',
    name: 'Baraza TV Deep House',
    url:
        'https://rtmp.streams.ovh:1936/barazarelax/barazazararelax/barazarelax/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/TZ1unwF.png',
  ),

  //Guadeloupe
  ChannelModel(
    countryCode: 'gp',
    name: 'Radio TV Basse-Terre',
    url: 'https://vdo.ssl-stream.com:3412/stream/play.m3u8',
    imageUrl: 'https://i.imgur.com/1gGz3Uw.jpg',
  ),
  ChannelModel(
    countryCode: 'gp',
    name: 'RHT Guadeloupe',
    url:
        'https://edge12.vedge.infomaniak.com/livecast/ik:livehautetension/manifest.m3u8',
    imageUrl: 'https://i.imgur.com/lV8M3nn.jpg',
  ),

  //Guatemala
  ChannelModel(
    countryCode: 'gt',
    name: 'Aurora Media Films',
    url: 'https://cdn.streamhispanatv.net:3417/live/auroramflive.m3u8',
    imageUrl: 'https://i.imgur.com/DVC5w6H.png',
  ),

  //Guinea
  ChannelModel(
    countryCode: 'gn',
    name: 'Cavi TV',
    url: 'https://helga.iptv2022.com/Cavi_TV/index.m3u8',
    imageUrl: 'https://i.imgur.com/kwXzUwd.png',
  ),

  //Guyana
  ChannelModel(
    countryCode: 'gy',
    name: 'FL Sport',
    url:
        'https://tvsw5-hls.secdn.net/tvsw5-chorigin/play/prod-8ce1fcb47fa7474d84640eb766f0ef38/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/i8o7QEj.png',
  ),

  //Haiti
  ChannelModel(
    countryCode: 'ht',
    name: 'Fidele TV',
    url: 'https://59d39900ebfb8.streamlock.net/FIDELETV/FIDELETV/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/YtBZ0Lc/FIDELE-TV.png',
  ),
  ChannelModel(
    countryCode: 'ht',
    name: 'Radio Télé 4VEH',
    url: 'https://uni01rtmp.tulix.tv/4vehtv/4vehtv-firetv/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/AnOjqaS.png',
  ),

  //Honduras
  ChannelModel(
    countryCode: 'hn',
    name: '504 TV',
    url: 'https://i.imgur.com/c2HLI3k.png',
  ),

  //Hong Kong
  ChannelModel(
    countryCode: 'hk',
    name: 'Bloomberg TV Asia',
    url: 'https://bloomberg.com/media-manifest/streams/asia.m3u8',
    imageUrl: 'https://i.imgur.com/OuogLHx.png',
  ),
  ChannelModel(
    countryCode: 'hk',
    name: 'Bloomberg TV Asia Live Event',
    url: 'https://bloomberg.com/media-manifest/streams/asia-event.m3u8',
    imageUrl: 'https://i.imgur.com/OuogLHx.png',
  ),
  ChannelModel(
    countryCode: 'hk',
    name: '凤凰资讯',
    url: 'http://php.17186.eu.org/phtv/fhzx.m3u8',
    imageUrl: 'https://i.imgur.com/mt4h3VO.png',
  ),

  //Hungary
  ChannelModel(
    countryCode: 'hu',
    name: '16tv Budapest',
    url:
        'https://cloudfront44.lexanetwork.com:1344/freerelay/16tv.sdp/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/04fCzHl.png',
  ),
  ChannelModel(
    countryCode: 'hu',
    name: 'ESTV',
    url:
        'https://cloudfront44.lexanetwork.com:1344/relay01/HDE032.sdp/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/OCZ8CfK.png',
  ),

  //Iceland
  ChannelModel(
    countryCode: 'is',
    name: 'RÚV',
    url: 'https://ruv-web-live.akamaized.net/streymi/ruverl/ruverl.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/1/11/R%C3%9AV_ohf_2019_logo.svg/512px-R%C3%9AV_ohf_2019_logo.svg.png',
  ),
  ChannelModel(
    countryCode: 'is',
    name: 'Vísir',
    url: 'https://live.visir.is/hls-live/visir.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/07Vlt6i.jpg',
  ),

  //India
  ChannelModel(
    countryCode: 'in',
    name: '9XM',
    url: 'https://d2q8p4pe5spbak.cloudfront.net/bpk-tv/9XM/9XM.isml/index.m3u8',
    imageUrl: 'https://i.imgur.com/F17QtN2.png',
  ),
  ChannelModel(
    countryCode: 'in',
    name: 'Aaj Tak',
    url: 'https://feeds.intoday.in/aajtak/api/aajtakhd/master.m3u8',
    imageUrl: 'https://i.imgur.com/RpIdCB3.png',
  ),

  //Indonesia
  ChannelModel(
    countryCode: 'id',
    name: 'AKTV',
    url: 'https://e.siar.us/live/aktv.m3u8',
    imageUrl: 'https://i.imgur.com/DUY5WXf.png',
  ),

  //Iran
  ChannelModel(
    countryCode: 'ir',
    name: 'Alborz TV',
    url: 'https://hls.247box.live/hls/stream.m3u8',
    imageUrl: 'https://i.imgur.com/tiHdLlK.png',
  ),
  ChannelModel(
    countryCode: 'ir',
    name: 'Abalfadhl TV',
    url: 'https://t.northtelecom.org/ABALFADHLTV.m3u8',
    imageUrl: 'https://i.ibb.co/cCmjqtQ/logo-2.png',
  ),

  //Iraq
  ChannelModel(
    countryCode: 'iq',
    name: 'Afaq TV',
    url: 'http://63b03f7689049.streamlock.net:1935/live/1/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/t7CaTu3.png',
  ),

  //Ireland
  ChannelModel(
    countryCode: 'ie',
    name: 'Cúla4',
    url: 'https://d3eq0jseza7fm5.cloudfront.net/tg4_vod_3.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c0/Cula4_Purple_2022_%281%29.svg/512px-Cula4_Purple_2022_%281%29.svg.png',
  ),

  //Israel
  ChannelModel(
    countryCode: 'il',
    name: 'ch 21 ערוץ הקניות',
    url: 'https://shoppingil-rewriter.vidnt.com/index.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/he/b/b9/%D7%A1%D7%9E%D7%9C%D7%99%D7%9C_%D7%A2%D7%A8%D7%95%D7%A5_%D7%94%D7%A7%D7%A0%D7%99%D7%95%D7%AA.jpg',
  ),

  //Italy
  ChannelModel(
    countryCode: 'it',
    name: 'Adnkronos',
    url:
        'https://5e73cf528f404.streamlock.net/GR_sport/livestream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/mEoDAf1.png',
  ),
  ChannelModel(
    countryCode: 'it',
    name: 'Aurora Arte',
    url:
        'https://59d7d6f47d7fc.streamlock.net/auroraarte/auroraarte/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Y1Yq3iV.png',
  ),

//Ivory Coast
  ChannelModel(
    countryCode: 'ci',
    name: 'Alpha et Omega TV',
    url: 'https://video1.getstreamhosting.com:1936/8318/8318/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/s9EgQQI.jpg',
  ),

  //Jamaica
  ChannelModel(
    countryCode: 'jm',
    name: 'Caribbean Gospel TV',
    url:
        'https://5a1178b42cc03.streamlock.net:443/CGTV2024/CGTV2024/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/mVQ8J4k.png',
  ),
  ChannelModel(
    countryCode: 'jm',
    name: 'Fame95 FM',
    url: 'https://rjr-fame.akamaized.net/hls/live/2033820/RJR_FAME/master.m3u8',
    imageUrl: 'https://i.imgur.com/fvtF0qM.png',
  ),

  //Japan
  ChannelModel(
    countryCode: 'jp',
    name: 'AT-X',
    url: 'https://neetball.net/live/neet3.m3u8',
    imageUrl: 'https://i.imgur.com/turWday.png',
  ),
  ChannelModel(
    countryCode: 'jp',
    name: 'CGNTV Japan',
    url: 'https://cgntvliveweb-944.acs.wecandeo.com/ms/2850/944/index.m3u8',
    imageUrl: '',
  ),
  ChannelModel(
    countryCode: 'jp',
    name: 'NTV News24',
    url: 'https://n24-cdn-live.ntv.co.jp/ch02/index.m3u8',
    imageUrl: 'https://i.imgur.com/5LNZeVq.png',
  ),

  //Jordan
  ChannelModel(
    countryCode: 'jo',
    name: 'Haqiqa',
    url: 'https://jmc-live.ercdn.net/alhaqiqa/alhaqiqa.m3u8',
    imageUrl: 'https://i.imgur.com/I9SJ2kr.jpg',
  ),
  ChannelModel(
    countryCode: 'jo',
    name: 'Kaifa TV',
    url: 'hhttp://82.212.74.2:8000/live/7311.m3u8',
    imageUrl: 'https://i.imgur.com/zGGeSQg.pn',
  ),

  //Kazakhstan
  ChannelModel(
    countryCode: 'kz',
    name: 'Abai TV',
    url: 'https://abaitv-stream.qazcdn.com/abaitv/abaitv/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Nw2R7Dq.png',
  ),
  ChannelModel(
    countryCode: 'kz',
    name: 'Alau TV',
    url: 'https://otv.wotom.net/alau/stream.m3u8',
    imageUrl: 'https://i.imgur.com/gK6ZRzp.png',
  ),
  ChannelModel(
    countryCode: 'kz',
    name: 'Balapan TV',
    url:
        'https://balapantv-stream.qazcdn.com/balapantv/balapantv/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/2o5T0gi.png',
  ),
  ChannelModel(
    countryCode: 'kz',
    name: 'Ontústik',
    url: 'https://stream.kaztrk.kz/regional/shymkenttv/index.m3u8',
    imageUrl: 'https://i.imgur.com/xqr1hjN.png',
  ),
  ChannelModel(
    countryCode: 'kz',
    name: 'Semeı',
    url: 'https://stream.kaztrk.kz/regional/semeytv/index.m3u8',
    imageUrl: 'https://i.ibb.co/P9V0532/Captura.png',
  ),

  //Kenya
  ChannelModel(
    countryCode: 'ke',
    name: 'Citizen Extra',
    url:
        'https://74937.global.ssl.fastly.net/5ea49827ff3b5d7b22708777/live_40c5808063f711ec89a87b62db2ecab5/index.m3u8',
    imageUrl: 'https://i.imgur.com/16d8NJd.jpg',
  ),
  ChannelModel(
    countryCode: 'ke',
    name: 'Inooro TV',
    url:
        'https://74937-castr.akamaized.net/5ea49827ff3b5d7b22708777/live_cd93fa8063f411ecb28b5d4f40b51a46/index.m3u8',
    imageUrl: 'https://i.imgur.com/5qFJtmo.png',
  ),
  ChannelModel(
    countryCode: 'ke',
    name: 'Morning Cloud TV',
    url: 'https://webstreaming.viewmedia.tv/web_026/Stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Lxfd2ip.png',
  ),
  ChannelModel(
    countryCode: 'ke',
    name: 'Ramogi TV',
    url:
        'https://citizentv.castr.com/5ea49827ff3b5d7b22708777/live_9b761ff063f511eca12909b8ef1524b4/index.m3u8',
    imageUrl: 'https://i.imgur.com/N2Uz9mc.jpg',
  ),

  //Kosovo
  ChannelModel(
    countryCode: 'xk',
    name: 'TV Arbëria 1',
    url: 'https://yayin30.haber100.com/live/rtvarberia/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zLRzyVS.png',
  ),
  ChannelModel(
    countryCode: 'xk',
    name: 'Arbëria 2',
    url: 'https://ssh101.bozztv.com/ssh101/rtvarberia2/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zLRzyVS.png',
  ),
  ChannelModel(
    countryCode: 'xk',
    name: 'TV Arbëria 3 Fëmijë',
    url: 'https://ssh101.bozztv.com/ssh101/rtvarberia3/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zLRzyVS.png',
  ),
  ChannelModel(
    countryCode: 'xk',
    name: 'TV Arbëria 4 Muzikë',
    url: 'https://ssh101.bozztv.com/ssh101/rtvarberia4/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zLRzyVS.png',
  ),
  ChannelModel(
    countryCode: 'xk',
    name: 'TV Arbëria 5',
    url: 'https://ssh101.bozztv.com/ssh101/rtvarberia5/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zLRzyVS.png',
  ),

  //Kuwait
  ChannelModel(
    countryCode: 'kw',
    name: 'KTV 1',
    url:
        'https://kwtktv1ta.cdn.mangomolo.com/ktv1/smil:ktv1.stream.smil/chunklist.m3u8',
    imageUrl: 'https://i.imgur.com/kG0d8Ay.png',
  ),
  ChannelModel(
    countryCode: 'kw',
    name: 'KTV 2',
    url:
        'https://kwtktv2ta.cdn.mangomolo.com/ktv2/smil:ktv2.stream.smil/chunklist.m3u8',
    imageUrl: 'https://i.imgur.com/hB57mNh.png',
  ),
  ChannelModel(
    countryCode: 'kw',
    name: 'KTV Sport',
    url:
        'https://kwtspta.cdn.mangomolo.com/sp/smil:sp.stream.smil/chunklist.m3u8',
    imageUrl: 'https://i.imgur.com/R1hGX1d.png',
  ),

  //Kyrgyzstan
  ChannelModel(
    countryCode: 'kg',
    name: '312 Кино',
    url: 'http://176.126.166.43:1935/live/312kino/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/3m9LrLj/k7gIibH.png',
  ),
  ChannelModel(
    countryCode: 'kg',
    name: '312 TB',
    url: 'http://176.126.166.43:1935/live/312musik/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/9HMNzMz/JPqHwaC.png',
  ),
  ChannelModel(
    countryCode: 'kg',
    name: 'TV1 KG',
    url: 'http://212.2.225.30:1935/live/site.stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Mti9t9C.png',
  ),

  //Laos
  ChannelModel(
    countryCode: 'la',
    name: 'Brian TV',
    url: 'https://livefta.malimarcdn.com/ftaedge00/briantv.sdp/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zG1WNgU.jpg',
  ),

  //Lebanon
  ChannelModel(
    countryCode: 'lb',
    name: 'Caribbean Hot 7 TV',
    url:
        'https://586a9edea75f1.streamlock.net/caribbeanhot7tv/ch7-1.stream/playlist.m3u8',
    imageUrl: 'https://i.ibb.co/sjTS4xD/logo116x90-2.png',
  ),
  ChannelModel(
    countryCode: 'lb',
    name: 'Al Mayadeen',
    url:
        'https://mdnlv.cdn.octivid.com/almdn/smil:mpegts.stream.smil/playlist.m3u8',
    imageUrl: '',
  ),
  ChannelModel(
    countryCode: 'lb',
    name: 'ALWifak News TV',
    url: 'http://stream.spilebanon.com:1935/live/myStream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/jrQ4w0m.png',
  ),

  //Lesotho
  //Liberia
  ChannelModel(
    countryCode: 'ls',
    name: 'LNTV',
    url: 'http://185.59.221.131:8081/live/lntv/playlist.m3u8',
    imageUrl: '',
  ),

  //Libya`
  ChannelModel(
    countryCode: 'ly',
    name: 'Al Masar TV',
    url: 'https://master.starmena-cloud.com/hls/almasar.m3u8',
    imageUrl:
        'https://play-lh.googleusercontent.com/FHPrsr391mPFqKfTq6ZdvWPZNVfsZeTAVQ8rjQF-NosnRdNipUy5xa43RWkiF4tGcQ',
  ),

  //Lithuania
  ChannelModel(
    countryCode: 'lt',
    name: 'M-1',
    url: 'https://m-1.data.lt/m-1/smil:m-1.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/dGhizFW.png',
  ),

  //Luxembourg
  ChannelModel(
    countryCode: 'lu',
    name: 'apart TV',
    url:
        'https://2-fss-2.streamhoster.com/pl_124/206820-4338160-1/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/IBcMIcZ.png',
  ),
  ChannelModel(
    countryCode: 'lu',
    name: 'Chamber TV',
    url:
        'https://media02.webtvlive.eu/chd-edge/smil:chamber_tv_hd.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/YldMnap.jpg',
  ),

  ///Latvia lv

  ChannelModel(
    countryCode: 'lv',
    name: 'Re TV',
    url:
        'https://retv2132.cloudycdn.services/slive/_definst_/retv_retv_channel_5k7_42787_default_891_hls.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Z3cre5Xm.png',
  ),
  ChannelModel(
    countryCode: 'lv',
    name: 'TVNET',
    url: 'https://player.tvnet.lv/stream/amlst:61659/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/jzv1ssE.png',
  ),

  ///Libya ly
  ChannelModel(
    countryCode: 'ly',
    name: 'Al Masar TV',
    url: 'https://master.starmena-cloud.com/hls/almasar.m3u8',
    imageUrl:
        'https://play-lh.googleusercontent.com/FHPrsr391mPFqKfTq6ZdvWPZNVfsZeTAVQ8rjQF-NosnRdNipUy5xa43RWkiF4tGcQU',
  ),
  ChannelModel(
    countryCode: 'ly',
    name: 'February Channel',
    url:
        'https://b01c02nl.mediatriple.net/videoonlylive/mtfknklgwrlive/broadcast_5dc818c793576.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/WdMIpsA.png',
  ),

  ///Morooco ma
  ChannelModel(
    countryCode: 'ma',
    name: '2M Monde',
    url:
        'https://cdnamd-hls-globecast.akamaized.net/live/ramdisk/2m_monde/hls_video_ts_tuhawxpiemz257adfc/2m_monde.m3u8',
    imageUrl: 'https://i.imgur.com/MvpntzA.png',
  ),
  ChannelModel(
    countryCode: 'ma',
    name: 'Chada TV',
    url: 'https://chadatv.vedge.infomaniak.com/livecast/chadatv/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/inJyJBN.png',
  ),
  ChannelModel(
    countryCode: 'ma',
    name: 'Télé Maroc',
    url:
        'https://api.new.livestream.com/accounts/27130247/events/9197096/live.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/8/8f/Logotelemaroc.jpeg',
  ),

  ///Monaco mc
  ChannelModel(
    countryCode: 'mc',
    name: 'Superyacht TV',
    url: 'https://sy.wns.live/hls/stream.m3u8',
    imageUrl: 'https://i.imgur.com/cgjgjE1.png',
  ),

  ///Montenegro me

  ChannelModel(
    countryCode: 'me',
    name: 'TV Teuta',
    url: 'https://teutastream.com/tvteuta/tracks-v1a1/mono.m3u8',
    imageUrl: 'https://i.imgur.com/sWVqBuW.png',
  ),

  ///North Macedonia mk
  ChannelModel(
    countryCode: 'mk',
    name: 'Folk Klub',
    url: 'http://tv1.intv.mk:1935/live2/folkklub/index.m3u8',
    imageUrl: 'https://i.imgur.com/Seq7Od7.png',
  ),
  ChannelModel(
    countryCode: 'mk',
    name: 'INTV Australia',
    url: 'http://tv1.intv.mk:1935/live/intv/index.m3u8',
    imageUrl: 'https://i.imgur.com/1nJl0Pv.png',
  ),

  //Myanmar
  ChannelModel(
    countryCode: 'mm',
    name: 'Channel K',
    url: 'https://l1-xl1.myanmarnet.com/relay/channelk/ch1/stream.m3u8',
    imageUrl: 'https://i.imgur.com/6PqxuhF.png',
  ),
  ChannelModel(
    countryCode: 'mm',
    name: 'Mahar',
    url: 'https://tv.mahar.live/mahar/website.stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/ig0QECf.png',
  ),
  ChannelModel(
    countryCode: 'mm',
    name: 'MCS',
    url: 'https://l1-xl1.myanmarnet.com/relay2/mcs/ch1/stream.m3u8',
    imageUrl: '',
  ),
  ChannelModel(
    countryCode: 'mm',
    name: 'MNTV',
    url: 'https://l1-xl1.myanmarnet.com/relay/mntv/ch1/stream.m3u8',
    imageUrl: 'https://i.imgur.com/nssm7QK.png',
  ),

  ///Nigeria ng
  ChannelModel(
    countryCode: 'ng',
    name: 'AfroSport Nigeria',
    url: 'https://newproxy3.vidivu.tv/vidivu_afrosport/index.m3u8',
    imageUrl:
        'https://pbs.twimg.com/profile_images/1451668129042599936/Uh-Z6Sh1_400x400.jpg',
  ),
  ChannelModel(
    countryCode: 'ng',
    name: 'AMusic Channel',
    url: 'http://mn-nl.mncdn.com/amusictv/amusicsrt.stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/06zuf64.png',
  ),
  ChannelModel(
    countryCode: 'ng',
    name: 'Dove TV',
    url:
        'https://webstreaming-7.viewmedia.tv/web_093/Stream/live93_3/high/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/wOlMfCR.png',
  ),

  ///Norway no
  ChannelModel(
    countryCode: 'no',
    name: 'Canal Motor',
    url: 'https://digicom.hls.iptvdc.com/canalmotor/index.m3u8',
    imageUrl: 'https://i.imgur.com/nCAt3QN.png',
  ),
  ChannelModel(
    countryCode: 'no',
    name: 'TV Modum',
    url: 'https://cdn010.panaccess.com/5677_streams/TVMODUM/index.m3u8',
    imageUrl: 'https://i.imgur.com/kljua5X.png',
  ),

  ///Oman om

  ChannelModel(
    countryCode: 'om',
    name: 'Alistiqama TV',
    url: 'https://jmc-live.ercdn.net/alistiqama/alistiqama.m3u8',
    imageUrl: 'https://i.imgur.com/2FPtYZr.png',
  ),
  ChannelModel(
    countryCode: 'om',
    name: 'Oman Althakafia',
    url:
        'https://partwota.cdn.mgmlcdn.com/omcultural/smil:omcultural.stream.smil/chunklist.m3u8',
    imageUrl: 'https://i.imgur.com/b38wNPK.png',
  ),

  ///Pakistan pk
  ChannelModel(
    countryCode: 'pk',
    name: 'Al Ehsaan TV',
    url: 'https://cdn.bmstudiopk.com/alehsaan/live/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/hRzxiJ3.png',
  ),
  ChannelModel(
    countryCode: 'pk',
    name: 'Discover Pakistan',
    url: 'https://livecdn.live247stream.com/discoverpakistan/web/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/IJH47fJ.png',
  ),

  ///Portugal pt

  ChannelModel(
    countryCode: 'pt',
    name: 'Kuriakos TV',
    url: 'https://w1.manasat.com/ktv/smil:ktv.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/xdAW0D5.png',
  ),
  ChannelModel(
    countryCode: 'pt',
    name: 'ON FM',
    url: 'https://5ce9406b73c33.streamlock.net/ONFM/livestream/playlist.m3u8',
    imageUrl:
        'https://onfmwordpressfiles.ams3.digitaloceanspaces.com/wp-content/uploads/2023/03/17130121/logo-on-fm.png',
  ),
  ChannelModel(
    countryCode: 'pt',
    name: 'RTP International Asia',
    url: 'http://210.210.155.37/qwr9ew/s/s38/index.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/f/f7/RTP_Internacional_2016_%28Reduced_Version%29.svg/512px-RTP_Internacional_2016_%28Reduced_Version%29.svg.png',
  ),

  ///Philippines ph
  ChannelModel(
    countryCode: 'ph',
    name: 'Cinema One',
    url: 'https://cinemaone-abscbn-ono.amagi.tv/index.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/en/thumb/6/6d/Cinema_One_2013_logo.svg/512px-Cinema_One_2013_logo.svg.png',
  ),
  ChannelModel(
    countryCode: 'ph',
    name: 'Great Commission TV',
    url: 'http://45.32.115.103/live/livestream/index.m3u8',
    imageUrl: 'https://i.imgur.com/8cvikOo.png',
  ),
  ChannelModel(
    countryCode: 'ph',
    name: 'MYX Philippines',
    url: 'https://myxnola-abscbn-ono.amagi.tv/index.m3u8',
    imageUrl: 'https://i.imgur.com/pbHuUuV.png',
  ),

  ///Qatar qa
  ChannelModel(
    countryCode: 'qa',
    name: 'Al Jazeera Arabic',
    url: 'https://live-hls-v3-aja.getaj.net/AJA-V3/index.m3u8',
    imageUrl: 'https://i.imgur.com/7bRVpnu.png',
  ),
  ChannelModel(
    countryCode: 'qa',
    name: 'Al Jazeera Balkans',
    url: 'https://live-hls-web-ajb.getaj.net/AJB/index.m3u8',
    imageUrl: 'https://i.imgur.com/vMzCr8O.png',
  ),
  ChannelModel(
    countryCode: 'qa',
    name: 'Al Jazeera English',
    url:
        'https://d1cy85syyhvqz5.cloudfront.net/v1/master/7b67fbda7ab859400a821e9aa0deda20ab7ca3d2/aljazeeraLive/AJE/index.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/en/thumb/f/f2/Aljazeera_eng.svg/512px-Aljazeera_eng.svg.png',
  ),
  ChannelModel(
    countryCode: 'qa',
    name: 'Qatar Television',
    url: 'https://qatartv.akamaized.net/hls/live/2026573/qtv1/master.m3u8',
    imageUrl: 'https://i.imgur.com/N5RB4sp.png',
  ),

  ///Russia RU
  ChannelModel(
    countryCode: 'ru',
    name: '7 TV',
    url: 'http://v4.proofix.ru/7tv-live/tracks-v1a1/index.m3u8',
    imageUrl: 'https://i.imgur.com/GpCwqUF.png',
  ),
  ChannelModel(
    countryCode: 'ru',
    name: 'Kulinar TV Show',
    url: 'https://api.alpaca.t62a.com/hls/9110/index.m3u8',
    imageUrl: 'https://tv.taurer.org/iptv_content/logos/KULINAR_TEMP.png',
  ),
  ChannelModel(
    countryCode: 'ru',
    name: 'Ocean TV',
    url: 'http://camera.global-x.ru/OceanTV/index.m3u8',
    imageUrl: 'https://i.imgur.com/zswzDZS.png',
  ),
  ChannelModel(
    countryCode: 'ru',
    name: 'Ювелирочка ТВ',
    url:
        'https://live-uvelirochka.cdnvideo.ru/uvelirochka/uvelirochka_720p3/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/IcWtXCZ.png',
  ),

  ///Saudi Arabia sa
  ChannelModel(
    countryCode: 'sa',
    name: 'Al Ekhbariya',
    url:
        'https://al-ekhbaria-prod-dub.shahid.net/out/v1/d443f3203b444032896e3233cb6eaa84/index.m3u8',
    imageUrl: 'https://i.imgur.com/WcRlHQm.png',
  ),
  ChannelModel(
    countryCode: 'sa',
    name: 'Asharq Discovery',
    url:
        'https://svs.itworkscdn.net/asharqdiscoverylive/asharqd.smil/playlist_dvr.m3u8',
    imageUrl: 'https://i.imgur.com/Czxi7yk.png',
  ),
  ChannelModel(
    countryCode: 'sa',
    name: 'Asharq Documentary',
    url:
        'https://svs.itworkscdn.net/asharqdocumentarylive/asharqdocumentary.smil/playlist_dvr.m3u8',
    imageUrl:
        'https://yt3.googleusercontent.com/AFUZ79or1rvrFKbfaMDdoJ1n_814lANCdUOdH0mdGRmcYfIfDH4cfTWwyUkHwie2I2xJPlaTJg=s512-c-k-c0x00ffffff-no-rj',
  ),
  ChannelModel(
    countryCode: 'sa',
    name: 'Asharq News',
    url:
        'https://shls-asharq-prod-dub.shahid.net/out/v1/3b6b4902cf8747a28619411239584002/index.m3u8',
    imageUrl: 'https://i.imgur.com/bqRC3sl.png',
  ),

  ///Singapore sg
  ChannelModel(
    countryCode: 'sg',
    name: 'CNA International',
    url:
        'https://d2e1asnsl7br7b.cloudfront.net/7782e205e72f43aeb4a48ec97f66ebbe/index.m3u8',
    imageUrl: 'https://i.imgur.com/xWglicB.png',
  ),
  ChannelModel(
    countryCode: 'sg',
    name: 'ZooMoo Asia',
    url: 'https://cdn4.skygo.mn/live/disk1/Zoomoo/HLSv3-FTA/Zoomoo.m3u8',
    imageUrl: 'https://i.imgur.com/ciTJrnl.png',
  ),

  ///South Korea kr
  ChannelModel(
    countryCode: 'kr',
    name: 'ABN TV',
    url: 'https://vod2.abn.co.kr/IPHONE/abn.m3u8',
    imageUrl: 'https://www.abn.co.kr/resources/images/logo.png',
  ),
  ChannelModel(
    countryCode: 'kr',
    name: 'CGNTV',
    url: 'https://cgntvliveweb-998.acs.wecandeo.com/ms/2850/998/index.m3u8',
    imageUrl: 'https://i.imgur.com/zwZASOB.png',
  ),
  ChannelModel(
    countryCode: 'kr',
    name: 'CTS기독교TV',
    url:
        'https://d34t5yjz1ooymj.cloudfront.net/out/v1/875039d5eba0478fa8375a06b3aa5a37/index.m3u8',
    imageUrl: 'https://i.imgur.com/z3L8uNB.png',
  ),

  ///Somalia so
  ChannelModel(
    countryCode: 'so',
    name: 'Afriwood Series',
    url:
        'https://origin3.afxp.telemedia.co.za/PremiumFree/afriwoodseries/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/9NFg3xp.png',
  ),
  ChannelModel(
    countryCode: 'so',
    name: 'Cinema Hausa',
    url: 'https://ebsonair.ebs.co.kr/ebs1familypc/familypc1m/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/uV5cHDW.png',
  ),
  ChannelModel(
    countryCode: 'so',
    name: 'Diva',
    url: 'https://origin3.afxp.telemedia.co.za/PremiumFree/diva/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/bgBZgW0.png',
  ),
  ChannelModel(
    countryCode: 'so',
    name: 'Sports Connect',
    url:
        'https://origin3.afxp.telemedia.co.za/PremiumFree/sportsconnect/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/0sNWg54.png',
  ),

  ///Taiwan tw
  ChannelModel(
    countryCode: 'tw',
    name: 'Beautiful Life TV',
    url: 'https://5ddce30eb4b55.streamlock.net/bltvhd/bltv1/playlist.m3u8',
    imageUrl: 'Beautiful Life TV',
  ),
  ChannelModel(
    countryCode: 'tw',
    name: 'Dali TV',
    url: 'http://www.dalitv.com.tw:4568/live/dali/index.m3u8',
    imageUrl: 'https://i.imgur.com/pFIMLsA.png',
  ),

  ///Tanzania tz

  ChannelModel(
    countryCode: 'tz',
    name: 'IBN TV',
    url: 'http://138.68.138.119:8080/live/5a8993709ea19/index.m3u8',
    imageUrl: 'https://i.imgur.com/y3zq0rw.jpg',
  ),
  ChannelModel(
    countryCode: 'tz',
    name: 'Abood TV',
    url: 'https://goliveafrica.media:9998/live/644db46d1b776/index.m3u8',
    imageUrl: 'https://i.imgur.com/vYEUncC.jpg',
  ),

  ///Ukraine ua
  ChannelModel(
    countryCode: 'ua',
    name: '24 Канал',
    url:
        'https://streamvideol1.luxnet.ua/news24/smil:news24.stream.smil/playlist.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/a/ab/24_Group_Ukraine_04.png/512px-24_Group_Ukraine_04.png',
  ),

  ChannelModel(
    countryCode: 'ua',
    name: 'BamBarBia TV',
    url: 'http://cdn1.live-tv.od.ua:8081/bbb/bbbtv-abr/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/zdRvy87.png',
  ),
  ChannelModel(
    countryCode: 'ua',
    name: 'Black Sea TV',
    url: 'https://stream.skyvision.com.ua/blackseatv/stream/main/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/D8YJzHV.png',
  ),

  ///Uganda ug

  ChannelModel(
    countryCode: 'ug',
    name: 'Alpha Digital',
    url:
        'http://streamfi-alphadgtl1.zettawiseroutes.com:8080/hls/stream_src.m3u8',
    imageUrl: 'https://i.imgur.com/ZlMARQx.jpg',
  ),
  ChannelModel(
    countryCode: 'ug',
    name: 'BBS TV',
    url:
        'https://cdn1.rea.cdn.moderntv.eu/readymedia/stream/BBSTV/5-hls/live-media.m3u8',
    imageUrl: 'https://i.imgur.com/1CIxIGl.png',
  ),
  ChannelModel(
    countryCode: 'ug',
    name: 'Bukedde TV 1',
    url: 'https://stream.hydeinnovations.com/bukedde1flussonic/index.m3u8',
    imageUrl: 'https://i.imgur.com/HFq5QlJ.png',
  ),
  ChannelModel(
    countryCode: 'ug',
    name: 'KSTV Uganda',
    url:
        'https://readymedia.moderntv.cloud:8092/distribute/readymedia/KS_TV_stream_dc/5-20-hls/live.m3u8',
    imageUrl: 'https://i.imgur.com/8w1xyHn.png',
  ),

  ///United Kingdom uk

  ChannelModel(
    countryCode: 'uk',
    name: 'BBC News (North America)',
    url: 'https://d2vnbkvjbims7j.cloudfront.net/containerA/LTN/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/vSz2WEp.png',
  ),
  ChannelModel(
    countryCode: 'uk',
    name: 'BEN Television',
    url: 'https://webstreaming-11.viewmedia.tv/web_153/Stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/DUAjjvT.png',
  ),
  ChannelModel(
    countryCode: 'uk',
    name: 'Bloomberg TV Europe',
    url: 'https://bloomberg.com/media-manifest/streams/eu.m3u8',
    imageUrl: 'https://i.imgur.com/OuogLHx.png',
  ),
  ChannelModel(
    countryCode: 'uk',
    name: 'GB News',
    url:
        'https://live-gbnews.simplestreamcdn.com/live5/gbnews/bitrate1.isml/manifest.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/en/thumb/3/35/GB_News_Logo.svg/512px-GB_News_Logo.svg.png',
  ),

  ///United States us

  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 1',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023560/abcnews1/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 2',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023561/abcnews2/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),

  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 3',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023562/abcnews3/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 4',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023563/abcnews4/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 5',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023564/abcnews5/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 6',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023565/abcnews6/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 7',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023566/abcnews7/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 8',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023567/abcnews8/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 9',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023568/abcnews9/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABC News Live 10',
    url:
        'https://abcnews-streams.akamaized.net/hls/live/2023569/abcnews10/master.m3u8',
    imageUrl: 'https://od.lk/s/MF8yMjk3NjU0OTJf/ABCNewsLive_2021_200x200.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'Adult Swim Channel 5',
    url: 'https://adultswim-vodlive.cdn.turner.com/live/channel-5/stream.m3u8',
    imageUrl: 'https://i.imgur.com/G9TyeCN.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: '30A Georgia Hollywood Review TV',
    url: 'https://30a-tv.com/gh.m3u8',
    imageUrl:
        'https://images.axios.com/JiG3RuYBwpU_WZFeU6HHjt03FAU=/111x0:1191x1080/320x320/2023/11/09/1699558891265.jpg',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABN China',
    url: 'https://mediaserver.abnvideos.com/streams/abnchina.m3u8',
    imageUrl: 'https://i.imgur.com/zagVLQH.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'ABN TV India',
    url: 'https://mediaserver.abnvideos.com/streams/abntvindia.m3u8',
    imageUrl: 'https://i.imgur.com/VcZn60X.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'Beach TV Panama City',
    url: 'http://media4.tripsmarter.com:1935/LiveTV/BTVHD/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/HCQDvEe.png',
  ),
  ChannelModel(
    countryCode: 'us',
    name: 'Bloomberg TV+ UHD',
    url:
        'https://bloomberg-bloombergtv-1-us.samsung.wurl.tv/manifest/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/OuogLHx.png',
  ),

  ChannelModel(
    countryCode: 'us',
    name: 'FOX News Now',
    url: 'https://fox-foxnewsnow-samsungus.amagi.tv/playlist.m3u8',
    imageUrl:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/6/67/Fox_News_Channel_logo.svg/512px-Fox_News_Channel_logo.svg.png',
  ),

  ///Uruguay uy

  ChannelModel(
    countryCode: 'uy',
    name: 'UCL',
    url: 'https://livedelta.cdn.antel.net.uy/out/u/url_canalu.m3u8',
    imageUrl: 'https://i.imgur.com/BzQphbw.png',
  ),

  /// Venezuela ve
  ChannelModel(
    countryCode: 've',
    name: 'Aguacate TV',
    url: 'https://streamtv.intervenhosting.net:3040/hybrid/play.m3u8',
    imageUrl: 'https://i.ibb.co/wpWBsgf/IMG-20230705-154622.jpg',
  ),
  ChannelModel(
    countryCode: 've',
    name: 'BTA TV',
    url: 'https://cloud.fastchannel.es/manifiest/hls/prog9/btatv.m3u8',
    imageUrl: 'https://i.imgur.com/vygOosC.png',
  ),
  ChannelModel(
    countryCode: 've',
    name: 'Cantina TV',
    url: 'https://vcp.myplaytv.com/cantinatv/cantinatv/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/JWMk2fP.png',
  ),

  ///Vietnam vn
  ChannelModel(
    countryCode: 'vn',
    name: 'Bac Ninh TV',
    url: 'https://stream.thingnet.vn/live/smil:BTV.smil/playlist.m3u8',
    imageUrl: 'https://i.postimg.cc/WbrWPpVg/BTV-Bac-Ninh-2021.png',
  ),
  ChannelModel(
    countryCode: 'vn',
    name: 'Cao Bang TV',
    url: 'http://118.107.85.4:1935/live/smil:CRTV.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/dGonewm.png',
  ),
  ChannelModel(
    countryCode: 'vn',
    name: 'Dhammasapha TV',
    url:
        'https://livefta.malimarcdn.com/ftaedge00/dhammasapha.stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/TWJgMuE.jpg',
  ),
  ChannelModel(
    countryCode: 'vn',
    name: 'Ha Tinh TV',
    url: 'https://wse.hatinhtv.net/live/httv1/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/4yIgKKx.png',
  ),
  ChannelModel(
    countryCode: 'vn',
    name: 'Little Saigon TV',
    url: 'https://media.streambrothers.com:1936/8228/8228/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/XyhgeKL.png',
  ),
  ChannelModel(
    countryCode: 'vn',
    name: 'Thai Nguyen TV',
    url: 'https://streaming.thainguyentv.vn/hls/livestream.m3u8',
    imageUrl:
        'https://thainguyentv.vn/modules/frontend/themes/ptthtn/images/logo.png',
  ),
  ChannelModel(
    countryCode: 'vn',
    name: 'Tien Giang',
    url: 'http://thtg.vn:8001/thtg.m3u8',
    imageUrl: 'https://i.imgur.com/yonKxgZ.png',
  ),

  ///Yemen ye
  ChannelModel(
    countryCode: 'ye',
    name: 'Almahriah TV',
    url: 'https://master.starmena-cloud.com/hls/almahriah.m3u8',
    imageUrl: '',
  ),
  ChannelModel(
    countryCode: 'ye',
    name: 'Yemen Today TV',
    url: 'https://video.yementdy.tv/hls/yementoday.m3u8',
    imageUrl: 'https://i.imgur.com/GgEeTNr.png',
  ),

  ///South Africa za
  ChannelModel(
    countryCode: 'za',
    name: '1KZN TV',
    url:
        'https://cdn.freevisiontv.co.za/sttv/smil:1kzn.stream.smil/playlist.m3u8',
    imageUrl:
        'https://admango.cdn.mangomolo.com/analytics/uploads/188/6544bebaae.jpg',
  ),
  ChannelModel(
    countryCode: 'za',
    name: 'Afrobeats TV',
    url: 'https://stream.ecable.tv/afrobeats/index.m3u8',
    imageUrl: 'https://i.imgur.com/u0d5j0C.png',
  ),
  ChannelModel(
    countryCode: 'za',
    name: 'Cape Town TV',
    url:
        'https://cdn.freevisiontv.co.za/sttv/smil:ctv.stream.smil/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/Y8FK1Kk.png',
  ),
  ChannelModel(
    countryCode: 'za',
    name: 'GOD TV Africa',
    url: 'https://webstreaming.viewmedia.tv/web_006/Stream/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/hW4g2oe.png',
  ),
  ChannelModel(
    countryCode: 'za',
    name: 'Hilaal TV',
    url:
        'https://cdn5.iqsat.net/iq/aa89b15058a61b904359307cc0a5e80a.sdp/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/tlylDbi.png',
  ),
  ChannelModel(
    countryCode: 'za',
    name: 'SABC News',
    url: 'http://185.59.221.131:8081/live/sabcnews/playlist.m3u8',
    imageUrl: 'https://i.imgur.com/liLta8j.png',
  ),
  ChannelModel(
    countryCode: 'za',
    name: 'Soweto TV',
    url:
        'https://cdn.freevisiontv.co.za/sttv/smil:soweto.stream.smil/playlist.m3u8',
    imageUrl:
        'https://admango.cdn.mangomolo.com/analytics/uploads/188/654228e5b5.jpg',
  ),
];
